...
| **Internal comments** | headreg 3s timeout (actual 8s); `crate::Runtime` (deleted); `app/<name>` (actual `app.<name>`) | — |
...
| **K3 — NEXT** | Proxy re-encryption ops (PRE rekey + reencrypt) | capability | **sharing / grants** (the whole substrate) | `cipher` already has **Umbral — a THRESHOLD PRE**: encrypt once, grant to a recipient via M-of-N re-encryption key fragments across proxy nodes (who never see plaintext), revocable by rotation. The threshold secret-sharing is *inside* Umbral, so K3 gives delegated **and** threshold sharing in one — **K4 is not needed for sharing.** Scope: expose `rekey`/`reencrypt` as host fns; grants are policy on top. |
...
- **Sharing [K3] — the encrypted-grants substrate (the next primitive build).** Expose `cipher`'s Umbral (a **threshold PRE**) as host fns: encrypt once, grant to recipients via M-of-N re-encryption across proxy nodes (who never see plaintext), revocable by rotation. Delegated + threshold sharing in one primitive — K4 is not needed for it.
...
2. **Sharing [K3] — the next primitive build (teed up).** Expose `cipher`'s Umbral (a threshold PRE, already in-tree) as host fns → encrypted, delegated, revocable grants; the cheapest remaining primitive and the whole sharing substrate (threshold is built into Umbral → K4 not needed). Scope: `rekey` + `reencrypt` host fns under a `Pre` capability (app profile), grants as policy. Then: PDP [K5] as the cryptographer-gated lattice-LHS milestone (`../binfield-lhs`); the funded-tier/economic layer (still unbuilt); K1 anchor half if/when a governed-WASM program appears. (K4 deferred; K6/K7/K9/K10 done + live.)
