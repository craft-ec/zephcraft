...
| **K7** | **Attestation** — user-defined named-quorum **authority** ("set up your own consensus") | wire | app-scoped authority; policy/grant sign-off | **DONE + LIVE 2026-07-14.** Governance GENERALIZED + UNIFIED: `Quorum`/`Attestation`/`QuorumChain` in `zeph-com` are the shared substrate, and `GovernanceSet = Quorum` (wire-identical) — **network governance is now a literal instance**, no duplicated k-of-n. The `attest` capability/host fn + `AttestBackend` + a per-program `AttestStore` (quorum chains) + the `attest-*` control-plane CLI (Package A: manual cosign; `attest()` = `is_authorized`). Rolled staggered (additive); live-validated: a 2-of-2 quorum authorized a statement across two nodes → an `attest()`-app committed `[01]` (and `[00]` for an unauthorized statement). Distinct from K6 (authority, not consistency); the two compose in the program. Deferred: `tag::ATTEST` gossip; member policy-program auto-signing. See `VERIFICATION_ATTESTATION_MODEL.md`. |
