...
| **K6** | **Verification** — cross-node re-execution + signed verdict; app-declared threshold `k` (1/2/3) | wire | compute **consistency** (is the output correct?) | Runtime re-executes deterministically already; missing the open request-board + verdict-collection protocol. Interchangeable open verifiers; higher `k` = more independent re-runs. |
