...
/// M2.3b: publish + get through the running daemon via a tracker — no manual
/// peer address. A publisher daemon (with storage-node daemons + a tracker)
/// publishes a file; a fetcher daemon retrieves it BY CID ALONE, byte-
/// identical. Exercises the whole obj+store+routing wiring end to end.
#[test]
fn publish_and_get_through_daemon_via_tracker() {
    let timeout = Duration::from_secs(60);

    // Tracker (separate binary in the zeph-tracker package).
    ...
    let taddr = wait_for(&tracker, "TRACKER_ADDR ", timeout)
        .expect("tracker addr")
        .trim()
        .strip_prefix("TRACKER_ADDR ")
        .unwrap()
        .to_string();

    // 3 storage-node daemons pointed at the tracker.
    let sdirs: Vec<tempfile::TempDir> = (0..3).map(|_| tempfile::tempdir().unwrap()).collect();
    ...
    for dir in &sdirs {
        ...
        wait_for(&w, "zeph node started", timeout);
        storage.push(w);
    }
    std::thread::sleep(Duration::from_secs(2)); // let storage nodes announce

    // Publisher daemon.
    let pdir = tempfile::tempdir().unwrap();
    let publisher = spawn_zeph(&[
        "--data-dir",
        pdir.path().to_str().unwrap(),
        "--reach",
        "local",
        "--tracker",
        &taddr,
    ...
    wait_for(&publisher, "zeph node started", timeout);

    let payload: Vec<u8> = (0..300_000u32)
        .map(|i| (i.wrapping_mul(2654435761) >> 13) as u8)
        .collect();
    let input = pdir.path().join("input.bin");
    std::fs::write(&input, &payload).unwrap();
    let out = Command::new(env!("CARGO_BIN_EXE_zeph"))
        .args([
            "publish",
            input.to_str().unwrap(),
            "--data-dir",
    ...
    let cid = stdout
        .lines()
        .find_map(|l| l.strip_prefix("ZEPH_CID "))
        .expect("ZEPH_CID printed")
        .trim()
        .to_string();

    // Fetcher daemon (fresh identity), same tracker; get BY CID ALONE.
    let fdir = tempfile::tempdir().unwrap();
    let fetcher = spawn_zeph(&[
        "--data-dir",
        fdir.path().to_str().unwrap(),
        "--reach",
        "local",
        "--tracker",
        &taddr,
    ...
    wait_for(&fetcher, "zeph node started", timeout);

    let restored = fdir.path().join("restored.bin");
    let out = Command::new(env!("CARGO_BIN_EXE_zeph"))
        .args([
            "get",
            &cid,
    ...
    drop((tracker, publisher, fetcher, storage));
}

/// Path to the tracker binary (different package), building it if absent.
fn tracker_exe() -> std::path::PathBuf {
    let zeph = std::path::PathBuf::from(env!("CARGO_BIN_EXE_zeph"));
    let tracker = zeph.parent().unwrap().join("tracker");
    if !tracker.exists() {
        let ok = Command::new(env!("CARGO"))
            .args(["build", "-p", "zeph-tracker"])
            .status()
            .map(|s| s.success())
            .unwrap_or(false);
        assert!(ok && tracker.exists(), "tracker binary must build");
    }
    tracker
}

/// Spawn the tracker binary (from the zeph-tracker package).
fn spawn_tracker(args: &[&str]) -> Worker {
    let program = tracker_exe();
